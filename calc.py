capcodd, capcode, capkodd, capcods, kapcod, capcot = [(-150, 120, "lightblue", "7"), (-50, 120, "lightblue", "8"), (50, 120, "lightblue", "9"),(-150, 50, "lightblue", "4"), (-50, 50, "lightblue", "5"), (50, 50, "lightblue", "6"),(-150, -20, "lightblue", "1"), (-50, -20, "lightblue", "2"), (50, -20, "lightblue", "3"),(-150, -90, "lightblue", "0"), (-50, -90, "lightblue", "."), (50, -90, "orange", "="),(150, 120, "orange", "+"), (150, 50, "orange", "-"), (150, -20, "orange", "*"), (150, -90, "orange", "/"),(-150, -160, "red", "C")], lambda *args: (__import__('functools').reduce(lambda x, y: x + y, [*args], 0) if (lambda x=iter(args): all(map(lambda y: isinstance(y, int) or isinstance(y, float), x))) else None), {'+': 'capcode','*': 'capcods','/': 'capcot'}, lambda *args: (lambda capcode=__import__('functools').reduce, capcd=lambda x: all(map(lambda z: isinstance(z, (int, float)), x)), capcud=lambda x, y: sum([x for _ in range(int(abs(y * 10**6)))]) / 10**6 if y != 0 else 0: capcode(lambda x, y: capcud(x, y) if y > 0 else -capcud(x, -y), args, 1) if capcd(args) else None)(), [1], lambda a, b: (lambda capcd=lambda x: isinstance(x, (int, float)), check_zero=lambda x: x == 0: (lambda capcot=lambda x, y: sum([abs(y)**-1 for _ in range(int(abs(x) * 10**6))]) / 10**6 if y != 0 else float('inf'): capcot(abs(a), abs(b)) * (1 if (a >= 0) == (b >= 0) else -1) if capcd(a) and capcd(b) and not check_zero(b) else None)())()
def cabcod(cabcods):
    def capcodz(gapcod):
        capchod, capcood = [], ""
        ...
        if capcood: capchod.append(capcood)
...
cabkood = __import__("turtle").Screen();cabkood.title("Calculatrice du Chaos");cabkood.bgcolor("black");cabkood.setup(width=500, height=700);capcoodzz = __import__("turtle").Turtle();capcoodzz.hideturtle();capcoodzz.color("white");capcoodzz.penup();capcoodzz.goto(0, 250);capcoodzz.write("0", align="center", font=("Courier", 24, "bold"));captcod = ""
for cappcodd in range(len(capcodd)):
    ...
    def clic(x, y, btn=capcodd[cappcodd][3]):
        global captcod
        if len(captcod) < 100:
            if btn == "=":captcod = str(eval(cabcod(captcod)))
            elif btn == "C":captcod = ""
            else:captcod += btn
            capcoodzz.clear();capcoodzz.write(captcod, align="center", font=("Courier", 24, "bold"))
